...
To facilitate the automatic remediation of the security alerts generated by the analytics rules the SOAR capabilities of Microsoft Sentinel was leveraged. Firstly, Sentinel SOAR essentials solution which contains various basic palybooks to start with was installed from Content Hub. 

![image](Images/CHB.png)

Afterwards, one of the playbook (Send-basic-email) was configured to send an email containing the incident details to the team/persons -in-charge for swift response once the alert is fired. In this case, the playbook will be configured to send the incident details as an email to a member of the security team when an unusaul Bastion access is detected.
...
Afterwards, neccessary modification of the playbook was implemented to suit its application.

![image](Images/CHB4.png)

To ensure that the playbook is ran once the alert is generated, the analytics which generates the alert was modified. 
...
Afterwards, an alert was generated by the analytics rule
...
This project highlights the significance of SIEM and SOAR solutions in not only securing devices but also monitoring the effectiveness and resilience of security solutions over time. It also underscores the relevance of log collection from resources with the IT environment to monitor their security status and implement the remediation of threat when they occur. Lastly, the integration of this security operation (SecOps) with the last project will no doubt enable comprehensive cloud workload protection. 

## Future Works

Having explored some of the key areas of Microsoft Sentinel including log ingestion, KQL command, analytics rule, incident investigation and automation with playbooks during this project and being able to complete the tasks involved efficiently, it indicates my familiarity and experience with Microsoft Sentinel as a tool for cybersecurity analysis. Moving on, further projects will be completed on incident investigation and the implementation of playbooks for advanced security operation.
